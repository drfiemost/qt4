...
{
    QString data;
    for (int i = 0; i < std::min<int>(MAX_DATA_DUMP, buf.size()); ++i) {
...
bool QSocks5SocketEngine::bind(const QHostAddress &address, quint16 port)
{
    ...
    if (d->socks5State == QSocks5SocketEnginePrivate::BindSuccess) {
        ...
        if (!dummy.bind()
            || writeDatagram(nullptr,0, d->data->controlSocket->localAddress(), dummy.localPort()) != 0
...
qint64 QSocks5SocketEngine::read(char *data, qint64 maxlen)
{
    ...
    if (d->mode == QSocks5SocketEnginePrivate::ConnectMode) {
        ...
        qint64 copy = std::min<qint64>(d->connectData->readBuffer.size(), maxlen);
...
qint64 QSocks5SocketEngine::write(const char *data, qint64 len)
{
    ...
    if (d->mode == QSocks5SocketEnginePrivate::ConnectMode) {
        ...
        len = std::min<qint64>(len, MaxWriteBufferSize) - d->data->controlSocket->bytesToWrite();
        if (len <= 0)
            return 0;
                                        ...
                                        quint16 *port)
{
    ...
    int copyLen = std::min<int>(maxlen, datagram.data.size());
