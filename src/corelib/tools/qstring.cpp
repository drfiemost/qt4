...
QByteArray toLatin1_helper(const QChar *data, int length)
{
...
QString &QString::insert(int i, QLatin1String str)
{
    ...
    std::memmove(d->data() + i + len, d->data() + i, (d->size - i - len) * sizeof(QChar));
...
QString& QString::insert(int i, const QChar *unicode, int size)
{
    ...
    if (!less(s, d->data()) && less(s, d->data() + d->alloc)) {
        ...
        Q_CHECK_PTR(tmp);
        std::memcpy(tmp, s, size * sizeof(QChar));
...
#if !defined(QT_USE_ICU) && defined(Q_OS_UNIX)
