...
QTime fromStringImpl(const QString &s, Qt::DateFormat f, bool &isMidnight24)
{
    ...
    case Qt::SystemLocaleLongDate:
    {
        QLocale::FormatType formatType(f == Qt::SystemLocaleLongDate ? QLocale::LongFormat : QLocale::ShortFormat);
...
QString QDateTime::toString(Qt::DateFormat f) const
{
    ...
    if (f == Qt::ISODate) {
        ...
        case QDateTimePrivate::UTC:
            ...
            break;
        case QDateTimePrivate::OffsetFromUTC: {
            //int sign = d->m_offsetFromUtc >= 0 ? 1: -1;
...
QDataStream &operator<<(QDataStream &out, const QDateTime &dateTime)
{
    ...
    if (out.version() >= 7) {
        ...
        if (dateTime.d->spec == QDateTimePrivate::OffsetFromUTC) {
...
QDataStream &operator>>(QDataStream &in, QDateTime &dateTime)
{
    ...
    if (in.version() >= 7) {
        in >> ts;
        if (ts == qint8(QDateTimePrivate::OffsetFromUTC))
            in >> offset;
    }
