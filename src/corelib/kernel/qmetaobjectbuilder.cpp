...
QMetaMethodBuilder QMetaObjectBuilder::addSignal(const QByteArray& signature)
{
    int index = d->methods.size();
    d->methods.append(QMetaMethodBuilderPrivate
        (QMetaMethod::Signal, signature, QByteArray(), QMetaMethod::Public));
    return QMetaMethodBuilder(this, index);
}
                           ...
                           bool relocatable)
{
    ...
    if (buf) {
        static_assert(QMetaObjectPrivate::OutputRevision == 6, "QMetaObjectBuilder should generate the same version as moc");
...
{
    int size = buildMetaObject(d, nullptr, false);
...
{
    int size = buildMetaObject(d, nullptr, true);
    if (size == -1) {
        if (ok) *ok = false;
        return QByteArray();
    }
