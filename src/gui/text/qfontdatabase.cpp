...
struct QtFontStyle
{
    ...
    {
#if defined(Q_WS_X11)
        weightName = setwidthName = nullptr;
#endif // Q_WS_X11
    }
...
QtFontSize *QtFontStyle::pixelSize(unsigned short size, bool add)
{
    ...
    pixelSizes[count].encodings = nullptr;
#endif
#if defined(Q_WS_QWS)
...
public:
    QFontDatabasePrivate()
        : count(0), families(nullptr), reregisterAppFonts(false)
#if defined(Q_WS_QWS)
          , stream(0)
#endif
#if defined(Q_WS_WIN) && !defined(QT_NO_DIRECTWRITE)
...
struct QtFontDesc
{
    inline QtFontDesc() : family(nullptr), foundry(nullptr), style(nullptr), size(nullptr), encoding(nullptr), familyIndex(-1) {}
    QtFontFamily *family;
                              ...
                              const QString &styleName = QString())
{
    ...
    for ( int i = 0; i < foundry->count; i++ ) {
        ...
        int d = std::abs( styleKey.weight - style->key.weight );

        if ( styleKey.stretch != 0 && style->key.stretch != 0 ) {
            d += std::abs( styleKey.stretch - style->key.stretch );
        }
...
{
    QtFontEncoding *encoding = nullptr;
                         ...
                         QtFontDesc *desc, int force_encoding_id)
{
    ...
    for (int x = 0; x < family->count; ++x) {
        ...
        if (px != pixelSize) // close, but not exact, size match
            this_score += std::abs(px - pixelSize);
