...
void QMenuBarPrivate::popupAction(QAction *action, bool activateFirst)
{
    ...
    if (action->isEnabled() && action->menu()->isEnabled()) {
        ...
        pos = QPoint(std::max(pos.x(), screenRect.x()), std::max(pos.y(), screenRect.y()));
...
void QMenuBarPrivate::init()
{
    ...
    oldWindow = oldParent = nullptr;
#ifdef QT_SOFTKEYS_ENABLED
    menuBarAction = 0;
#endif
#ifdef Q_WS_X11
    cornerWidgetToolBar = nullptr;
    cornerWidgetContainer = nullptr;
#endif
    handleReparent();
...
*/
QMenuBar::QMenuBar(QWidget *parent) : QWidget(*new QMenuBarPrivate, parent, nullptr)
{
    Q_D(QMenuBar);
    d->init();
}
