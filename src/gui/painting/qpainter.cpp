                                 ...
                                 bool strikeOut)
{
    ...
    if (extended != nullptr && state->matrix.isAffine()) {
        ...
        std::memset(glyphAttributes.data(), 0, glyphAttributes.size() * sizeof(HB_GlyphAttributes));
        std::memset(static_cast<void *>(advances.data()), 0, advances.size() * sizeof(QFixed));
        std::memset(static_cast<void *>(glyphJustifications.data()), 0, glyphJustifications.size() * sizeof(QGlyphJustification));
...
void QPainter::drawText(const QPointF &p, const QString &str, int tf, int justificationPadding)
{
    ...
    if (tf & Qt::TextBypassShaping) {
        ...
        if (!fontEngine->stringToCMap(str.data(), len, &glyphs, &numGlyphs, nullptr)) {
                                                ...
                                                const QTextCharFormat &charFormat)
{
    ...
    QFixed width = rightMost - leftMost;
    QTextItem::RenderFlags flags = nullptr;
...
{
    QPaintDeviceRedirection() : device(nullptr), replacement(nullptr), internalWidgetRedirectionIndex(-1) {}
