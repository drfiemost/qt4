                    ...
                    const QString &signature_, QList<int>& metaTypes)
{
    ...
    for (int idx = mo->methodCount() - 1 ; idx >= QObject::staticMetaObject.methodCount(); --idx) {
        ...
        if (returnType != 0 && QDBusMetaType::typeToSignature(returnType) == nullptr)
            continue;
...
QDBusConnectionPrivate::QDBusConnectionPrivate(QObject *p)
    : QObject(p), ref(1), capabilities(nullptr), mode(InvalidMode), connection(nullptr), server(nullptr), busService(nullptr),
...
{
    QDBusConnection::ConnectionCapabilities result = nullptr;
...
int QDBusConnectionPrivate::send(const QDBusMessage& message)
{
    ...
    bool isOk;
    {
        ...
        isOk = q_dbus_connection_send(connection, msg, nullptr);
    }
    int serial = 0;
    if (isOk)
...
static void cleanupDeletedNodes(QDBusConnectionPrivate::ObjectTreeNode &parent)
{
    ...
    while (it.hasNext()) {
        ...
        if (node.obj == nullptr && node.children.isEmpty())
            it.remove();
        else
