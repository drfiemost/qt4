...
Q_DECLARE_METATYPE(QVector<Movable>);
...
Q_DECLARE_METATYPE(QVector<Custom>);
...
void tst_QVector::reallocAfterCopy_data()
{
    ...
    for (int i = 70; i <= 100; i += 10) {
        ...
        for (int j = 0; j <= 3; j++) {
            ...
            QTest::newRow(qPrintable(QString("reallocAfterCopy:%1").arg(i))) << i << fill_size << j << result1 << result2 << result3 << result4;
        }
    }
}
