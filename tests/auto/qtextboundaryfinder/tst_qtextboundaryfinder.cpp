                       ...
                       QTextBoundaryFinder::BoundaryType type, bool default_algorithm_only = false)
{
    ...
    for (int i = 0; i < testString.length(); ++i) {
        ...
        QCOMPARE(boundaryFinder.isAtBoundary(), bool(expectedBreakPositions.contains(i)));
    }
}
...
{
    generateDataFromFile(SRCDIR "data/GraphemeBreakTest.txt");
}
...
{
    generateDataFromFile(SRCDIR "data/WordBreakTest.txt");
}
...
{
    generateDataFromFile(SRCDIR "data/SentenceBreakTest.txt");
}
...
{
    generateDataFromFile(SRCDIR "data/LineBreakTest.txt");
}
